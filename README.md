...
结果按股票缓存在 `data/technical.json`，默认 24 小时刷新一次（`config.json` 里 `technical` 段可调并发、请求间隔与每轮上限；新浪偶发限流，失败记录 1 小时后自动重试）。云端把每轮上限设为 60 只，避免一次抓几百只被限流，其余顺延到下一轮补齐；GitHub Pages 同样继承上一轮公开缓存。
## 装成手机 App（PWA）
